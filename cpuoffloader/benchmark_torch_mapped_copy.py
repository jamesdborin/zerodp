...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Benchmark the pagellm::triton_mapped_expert_copy custom Triton kernel."
    )
    parser.add_argument(
        "--warmup-iters",
        type=int,
...
def _resolve_fp8_dtype(format_name: str) -> torch.dtype:
    match format_name.lower():
        ...
        case _:
            dtype = None
    if dtype is None:
        raise RuntimeError(f"Requested FP8 format '{format_name}' is not available in this PyTorch build.")
    return dtype
...
def _allocate_tensors(num_cpu_experts: int, dtype: torch.dtype) -> tuple[torch.Tensor, ...]:
    if num_cpu_experts < 0 or num_cpu_experts > TOTAL_EXPERTS:
        raise ValueError(f"num_cpu_experts must be in [0, {TOTAL_EXPERTS}], got {num_cpu_experts}.")
    ...
    combined = torch.empty((TOTAL_EXPERTS,) + EXPERT_SHAPE, dtype=dtype, device="cuda", requires_grad=False)
...
def _timed_kernel_launch(
    ...
    times_ms: list[float] = []
    num_cpu_experts = cpu_expert.shape[0]
    for _ in range(iterations):
        start = torch.cuda.Event(enable_timing=True)
        end = torch.cuda.Event(enable_timing=True)
        start.record()
        combined_expert[:num_cpu_experts].copy_(cpu_expert)
        end.record()
        end.synchronize()
...
def _sweep_offload_counts(counts: Iterable[int], dtype: torch.dtype, warmup_iters: int, bench_iters: int) -> None:
